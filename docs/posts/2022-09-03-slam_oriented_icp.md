...
现有多类 ICP 算法，例如从 point2point 到 point2line, point2surface，或借助多尺度多分辨率加速，或引入额外类型数据（例如色调）。
...
$$
对于对应关系未知的数据，我们需要“猜测”对应性。此类问题没有直接解法，需要使用启发式方法，例如数值牛顿法、遗传算法、以及常见针对 SLAM 系统研发的 ICP 算法变种（本文主要研究对象）。

## 3  基本 ICP 算法和以往分类方法

### 3.1 基本 ICP 算法

ICP 算法基本思想：若源点云和目标点云之间的对应关系已知，则可以方便地求解出两者的相对位姿。问题是猜测的对应关系是不确定的，因此我们需要在 while 循环中迭代执行猜测。
...
离线 SLAM 更注重精度，因此对 ICP 的效率要求相对宽松。
...
基于路标的 SLAM 利用在图片中检测到的路标信息，例如树木、建筑、障碍物、或人工路标，对 ICP 方法具有特别的要求，因为路标提供了高置信度的信息。
...
不基于路标的 SLAM 系统由于缺少特征，精度相对有限，因此研究方向主要是改善精度。
